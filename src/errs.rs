...
impl Errs{
    pub fn print(&self) -> &str {
        match self {
            Errs::ImpermissibleBehaviour => { return "Impermissible Behaviour! Syntax-analizer proccess was stopped"; },
            ...
            Errs::RightSideVar => { return "В правой части после переменной может стоять либо функция, либо математическая операция, либо метка в начале очередного блока оператор, либо терминал 'конец'"; },
            Errs::RightSideInt => { return "В правой части после целого числа может стоять либо функция, либо математическая операция, либо метка в начале очередного блока оператор, либо терминал 'конец'"; },
            ...
            Errs::RigthSideDoubleMathOp => {return "Недопустимо повторение аддитивных, мультиплекативных и логических операций";}
        }
    }
}